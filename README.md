...
- **菜单栏常驻**：最左侧小狗图案（SF Symbols `dog.fill`），随后按顺序展示 CPU / GPU / 内存 / 网络上下行；每个指标上下两行（数值在上、名称在下），列间用 `｜` 分隔，紧凑省宽度。
- **下拉菜单控制**：4 个开关（CPU / GPU / 内存 / 网络），勾选即显示、取消即隐藏；隐藏后完全不采集对应数据。网络的上行、下行共用一个开关。
...
- 网络列固定两行：上行在上、下行在下；
- 小狗用 SF Symbols `dog.fill`，垂直居中放在最左侧。

> 相关代码：`MenuBarLabelRenderer.swift`、`MenuBarLabelView.swift`。

### 3. 下拉菜单：`MenuBarContentView.swift`

- `Section` 分组显示 4 个 `Toggle`（CPU / GPU / 内存 / 网络），直接绑定 `AppSettings.binding(for:)`。
- `Divider()` 分隔开关与操作按钮。
...
- `percent(_:)`：`0.1234 → "12%"`，`nil → "--"`。
- `rate(_:)`：单位自动换档，**最小单位是 K/s**：`1536 → "1.5 K/s"`、`512 → "0.5 K/s"`、`0 → "0.0 K/s"`、`1572864 → "1.5 M/s"`；数值 ≥ 100 时不显示小数，保证菜单栏宽度稳定。
