...
enum MenuBarLabelRenderer {
    ...
    /// 菜单栏图标：把 1024px 的原图 App 图标一次性缩到菜单栏需要的尺寸（44px = 20pt 的 2 倍），
    /// 避免把整张高清原图解码进内存（省几 MB），之后每次绘制都复用这张小图。
    private static let clawImage: NSImage? = {
        guard let source = NSImage(named: NSImage.applicationIconName) else { return nil }
        let pixel: CGFloat = 44
        return NSImage(size: NSSize(width: pixel, height: pixel), flipped: false) { rect in
